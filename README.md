...
There are two ways to do the automatic generation of tests, in the first way we define the configuration in a class generated by ourselves, and in the second we use the automatic generation of this class.
...
the second step is to save the typeInfo in the configuration class, this in order to easily reuse the typeInfo and avoid generating it every time a different experiment is performed.

``` Smalltalk
GASSTeacherTest addTypeInfo: typeInfo withKey: 'steacher'
```

the third step is to reference the typeInfo by means of its key in the configuration class. Next we will define as typeInfo the one we saved above.
...
### 2.2. Generate tests from default

When a configuration class is not provided, the STestCaseFactory class will create one by default, for this you must first provide the name of the targetClass, and you can leave the configurations that are defined by default, below we present an example of this.
